    ...
    public boolean checkConnect(Point p1, Point p2) {
        ...
        if (p1.getX() != p2.getX()) {
            ...
            if (clearY(maxPos, minPos, p1.getY() < p2.getY() ? p1.getX() : p2.getX())) {
                for (y = maxPos + 1; y < sizeY; y++) {
                    if (grid[p1.getX()][y].getState() != TileState.REMOVED) {
                        break;
                    }
                    if (grid[p2.getX()][y].getState() != TileState.REMOVED) {
                        break;
                    }
        ...
        if (clearX(maxPos, minPos, p1.getX() < p2.getX() ? p1.getY() : p2.getY())) {
            for (x = maxPos + 1; x < sizeX; x++) {
                if (grid[x][p1.getY()].getState() != TileState.REMOVED) {
                    break;
                }
                if (grid[x][p2.getY()].getState() != TileState.REMOVED) {
                    break;
                }
